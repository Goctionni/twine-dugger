...
export function LockSettings() {
  ...
  const onClearAll = () => {
    const lockedPaths = sortPaths(getLockedPaths());
    ...
    <section class="rounded-md border border-gray-700 bg-gray-800/40 p-3">
      <div class="mb-2 flex items-center justify-between gap-3">
        <p class="text-sm text-gray-300">
          Locked paths: <span class="font-semibold text-gray-100">{lockedPaths.length}</span>
        </p>
        <button
          type="button"
          class={btnClass('clr-gray hover:clr-sky px-2 py-0.5 text-xs')}
          onClick={onClearAll}
          disabled={!lockedPaths.length}
        >
          Clear all
        </button>
      </div>
