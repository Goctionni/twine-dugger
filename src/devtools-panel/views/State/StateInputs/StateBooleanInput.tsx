...
export function StateBooleanInput(props: StateBooleanInputProps) {
  ...
  const getId = () => getPath().join('_').replaceAll(/[^a-zA-z0-9]/g, '_').replaceAll(/_{2,}/g, '_');
  ...
  return (
    <div class="flex justify-start select-none gap-2">
      <BooleanInput value={currentValue()} onChange={handleChange} disabled={isDisabled()} id={getId()} />
